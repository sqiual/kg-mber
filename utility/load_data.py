...
from utility.datasets import UIDataset,KGDataset
from config.configurator import configs
from utility.models import KGModel, Contrast, BPRLoss
...
class DataHandler(object):
    def __init__(self, dataset, batch_size):
        ...
        if self.dataset_name.find('Beibei') != -1 or self.dataset_name.find('Taobao') != -1 or self.dataset_name.find('retail_rocket') != -1:
            behs = ['pv','cart', 'train'] # ['pv', 'cart', 'train']
        elif self.dataset_name == 'tmall':
            behs = ['pv', 'fav', 'cart', 'train']
        elif self.dataset_name == 'ijcai_15':
    ...
    def LoadData(self):
        self._load_data()
        # kg-handler
        self.raw_kg_dataset = UIDataset(path=self.predir, tot_items = self.n_items, tot_users = self.n_users)
        self.kg_dataset = KGDataset(self.n_items)
