...
class KGDataset(Dataset):
    def __init__(self, m_item, kg_path = './dataset/' + args.dataset + '/kg.txt'):
        self.m_item = m_item 
    ...
    def generate_kg_data(self, kg_data):
        kg_dict = collections.defaultdict(list) # {[]} defaultdict(<class 'list'>, {})        
        # for row in tqdm(kg_data.iterrows()):
...
class UIDataset(BasicDataset):
    def __init__(self, path, tot_items, tot_users):
        ...
        self.m_item += 1 
        self.n_user += 1 
        self.train_n_user = self.n_user
        self.train_m_item = self.m_item
        self.m_item = tot_items
        self.n_user = tot_users
        self.Graph = None
